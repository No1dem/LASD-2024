...
public:
  ...
  StackLst(const TraversableContainer<Data>& TravCon) : List<Data>(TravCon) {}; // A stack obtained from a TraversableContainer
  StackLst(MappableContainer<Data>&& MapCon) : List<Data>(std::move(MapCon)) {}; // A stack obtained from a MappableContainer

  /* ************************************************************************ */

  // Copy constructor
  StackLst(const StackLst<Data>& stack) : List<Data>(stack) {};

  // Move constructor
  StackLst(StackLst<Data>&& stack) : List<Data>(std::move(stack)) {};
  ...
  inline bool operator!=(const StackLst<Data>& stack) const noexcept {return !(stack == *this);};
